    ...
    public class GearManager : Singleton<GearManager>
    {
        ...
        public bool IsGearPlaceableOnGridPoint(string gearKey, Vector2Int center)
        {
            ...
            foreach (var offset in localGridPoints)
            {
                ...
                if (!RTS_GridWorldSystem.Instance.GetGridNode(gridPoint).isMountable)
                {
                    return false;
                }
            }
        ...
        public Vector2 GridPointToWorldPos(Vector2Int gridPoint) => RTS_GridWorldSystem.Instance.GetWorldPosFromGrid(gridPoint);
        public Vector2Int WorldPosToGridPoint(Vector2 wrdPos) => RTS_GridWorldSystem.Instance.GetGridPointFromWorld(wrdPos);
        #endregion
    }
}