...
public class GearPlaceMarkerControl : MonoBehaviour
{
    ...
    async void OnValidateGridPointForGear(GearPlaceArg gearPlaceArg)
    {
        ...
        placeMarkerSprite.transform.position = RTSGridWorldSystem.Instance.GetWorldPosFromGrid(gearPlaceArg.snapPoint) + GearManager.Instance.GetGearPlaceOffset(gearPlaceArg.gearKey);
        if (RTSGridWorldSystem.Instance.GetGridNode(gearPlaceArg.snapPoint).isMountable)
        {
